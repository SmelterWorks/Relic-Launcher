...
Words, phrases, punctuation patterns (em dashes, semicolons), formatting habits (backtick overuse), structural signals, and statistical measures commonly associated with AI-generated text. Avoid these to ensure writing sounds natural and human.
...
- Semicolons: A Secondary Punctuation Tell
- Backtick Overuse in Prose
- Overused Verbs
- Overused Adjectives
...
**Semicolon overuse has joined em dashes as a reliable stylistic marker of AI-generated prose.**

Semicolons join related independent clauses. They are valid punctuation, but AI models reach for them far more often than human writers do in documentation, blog posts, and casual technical writing.

### Why Semicolons Signal AI Writing
- Models trained on edited formal prose treat semicolons as a sophistication marker
- AI joins independent clauses with semicolons where two short sentences or a comma plus conjunction would read more naturally
- Semicolon density in AI output often exceeds what the same writer would use in email, chat, or internal notes
- Heavy semicolon use combined with em dashes is a high-confidence stylistic cluster

### What To Do Instead
| Instead of | Use |
|------------|-----|
| The feature is optional; users can skip it | The feature is optional. Users can skip it. |
| We tested three builds; all passed | We tested three builds, and all passed. |
| The API is stable; breaking changes are rare | The API is stable. Breaking changes are rare. |

### Guidelines
- Default to a period when two clauses could stand alone
- Use commas with coordinating conjunctions for closely related ideas
- Reserve semicolons for rare cases where two clauses are tightly linked and a comma would cause ambiguity
- If you find more than one semicolon per few hundred words of casual prose, revise

---

## Backtick Overuse in Prose

**AI-generated markdown wraps too many words in backticks, treating inline code formatting as emphasis for any technical-sounding term.**

Backticks are for literals: commands, file paths, identifiers, and exact UI labels. AI models apply them indiscriminately to ordinary words that merely sound technical.

### Why Backticks Signal AI Writing
- Models equate backticks with "this is a technical term" and apply them to common nouns and concepts
- Product names, API concepts, and plain English words get backtick-wrapped even when unformatted text reads fine
- Dense backtick usage makes documentation read like a keyword index rather than human explanation
- Runs of multiple backtick-wrapped words in one sentence are rare in human writing but common in AI output

### What To Do Instead
| Instead of | Use |
|------------|-----|
| Configure the `authentication` `module` | Configure the authentication module |
| This affects the `user` `experience` | This affects the user experience |
| Update your `config` file | Update your config file (or `config.yaml` when naming a specific file) |
| The `deployment` `pipeline` handles `releases` | The deployment pipeline handles releases |

### Guidelines
- Backtick only literals: file names, commands, code identifiers, env vars, and exact UI labels
- Do not backtick ordinary English words just because they relate to software
- If removing backticks does not change meaning, leave them off
- Scan for runs of multiple backtick-wrapped words in a single sentence. That pattern is almost always AI.
...
- Semicolon overuse in prose where periods or commas would be more natural
- Extensive backtick wrapping of non-literal terms in markdown output
...
11. Count semicolons. More than one per few hundred words of casual prose warrants revision.
12. Scan for backtick clusters. Multiple backtick-wrapped words in one sentence usually means over-formatting.
