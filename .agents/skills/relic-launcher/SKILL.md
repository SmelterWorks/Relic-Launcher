...
- Sidebar nav: Home, Versions, Mods, Backup, Servers, Hosting, Wiki, Settings, About
...
- Hosting: Local tab on Windows/Linux (install server build, start/stop/restart, console I/O, folder shortcuts). Cloud tab shows SmelterWorks managed hosting plans (in-app purchase coming soon). macOS opens Cloud by default.
- Servers: public list browser (SmelterWorks proxy `RelicLauncherEndpoints.ServerListUrl` with official masterserver fallback), direct connect, LAN join via `--connect`, favorites and recents in Relic app data
